...
export const generateDates = (): GeneratedDates => {
  ...
  const updatedAt = new Date(createdAt.getTime() + faker.number.int({ min: 1000, max: 1000000 }) * 60 * 60 * 24 * 30);
  const deletedAt = randomBoolean() ? new Date(createdAt.getTime() + faker.number.int({ min: 1000, max: (updatedAt.getTime() - createdAt.getTime()) / 60000 }) * 60 * 1000) : null;
