...
const weightedRandomFlag = (): UserCommentFlag => {
  if (Math.random() < 0.8) {
    ...
    return faker.helpers.arrayElement(Object.values(UserCommentFlag).filter(flag => flag !== 'NONE'))
  }
}

const generateVotes = (userIds: string[], itemId: string, itemType: 'review' | 'comment'): {
...
export const generateReviews = async (
  ...
  for (let i = 0; i < count; i++) {
    ...
    generatedVotes.push(...votes.map(vote => ({
...
export const seedReviewsAndComments = async (count: number): Promise<void> => {
  ...
  const { comments, votes: commentVotes } = await generateComments(existingUserIds, reviewIds, count * 5)
