...
async function generateForcedTestUsers(): Promise<UserWithProfile[]> {
  ...
  // Admin
  forcedUsers.push(await UserFactory.createAdmin({ email: "admin@example.com", password, fullname: "Admin User" }));

  // Normal user
  forcedUsers.push(await UserFactory.createUser({ email: "normalUser@example.com", password, fullname: "Normal User" }));

  // Banned user
  const bannedUser = await UserFactory.createUser({ email: "bannedUser@example.com", password, fullname: "Banned User" });
  ...
  forcedUsers.push(await UserFactory.createUser({ email: "unverifiedEmailUser@example.com", password, fullname: "Unverified Email User" }));

  // Deleted user
  const deletedUser = await UserFactory.createUser({ email: "deletedUser@example.com", password, fullname: "Deleted User" });
  await prisma.user.update({
  ...
  // Verified Seller
  const verifiedSeller = await UserFactory.createSeller({ email: "verifiedSeller@example.com", password, fullname: "Verified Seller" });
  await prisma.user.update({
  ...
  // Unverified Seller
  forcedUsers.push(await UserFactory.createSeller({ email: "unverifiedSeller@example.com", password, fullname: "Unverified Seller" }));

  // Moderator
  forcedUsers.push(await UserFactory.createModerator({ email: "moderator@example.com", password, fullname: "Moderator User" }));

  return forcedUsers;
}

async function createUserWithSpecialCases(index: number, userData: { email: string; password: string; fullname: string }): Promise<UserWithProfile> {
  let userWithProfile: UserWithProfile;

  if (index % 10 === 0) { // Every 10th user is banned
    userWithProfile = await UserFactory.createUser(userData);
    await prisma.user.update({
      where: { id: userWithProfile.user.id },
      data: { bannedUntil: faker.date.future() }
    });
  } else if (index % 7 === 0) { // Every 7th user is deleted
    userWithProfile = await UserFactory.createUser(userData);
    await prisma.user.update({
      where: { id: userWithProfile.user.id },
      data: { deletedAt: new Date() }
    });
  } else if (index % 5 === 0) { // Every 5th user has a verified email
    userWithProfile = await UserFactory.createUser(userData);
    await prisma.user.update({
      where: { id: userWithProfile.user.id },
...
async function createSellerWithSpecialCases(index: number, userData: { email: string; password: string; fullname: string }): Promise<UserWithProfile> {
  let userWithProfile: UserWithProfile;

  if (index % 3 === 0) { // Every 3rd seller is verified
