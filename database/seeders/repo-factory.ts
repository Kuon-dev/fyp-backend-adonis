...
function selectSeller(
  ...
  const eligibleVerifiedSellers = verifiedSellers.filter(
    (seller) => (sellerRepoCounts.get(seller.id) || 0) < config.maxReposPerUser
  )
  const eligibleUnverifiedSellers = unverifiedSellers.filter(
    (seller) => (sellerRepoCounts.get(seller.id) || 0) < config.maxReposPerUser
  )
...
async function generateCodeRepos(count: number = 10, config: SeederConfig) {
  ...
  const verifiedSellers = allSellers.filter(
    (seller) => seller.sellerProfile && seller.sellerProfile.verificationStatus === 'APPROVED'
  ) as (User & { sellerProfile: SellerProfile; profile: Profile | null })[]

  const unverifiedSellers = allSellers.filter(
    (seller) => seller.sellerProfile && seller.sellerProfile.verificationStatus !== 'APPROVED'
  ) as (User & { sellerProfile: SellerProfile; profile: Profile | null })[]
  ...
  for (let i = 0; i < count; i++) {
    try {
      ...
      const status: CodeRepoStatus =
        Math.random() < config.activeRepoProbability
          ? 'active'
